describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    */
    for (const prodItem of prodItemsData) {
      if (prodItem.title.length == 0) { allArePopulated = false; break; }
      if (prodItem.price.length == 0) { allArePopulated = false; break; }
      if (prodItem.image.length == 0) { allArePopulated = false; break; }
    }
  }, 10000);
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (const prodItem of productItems) {
      if (counter1 == 0) { counter1++; continue; } //Skip first item, janky fix
    ...
    const cartCountNumber = await cartCountText.jsonValue();
